...
> IM 客户端在 iOS 端的原生宿主工程，承载 Flutter 引擎与「潮商会」应用在 iPhone / iPad 上的运行，负责推送、权限、后台模式、CallKit/Intent 等平台能力。
...
- **应用入口与配置**：`AppDelegate`、`Runner/Info.plist`、Bundle ID（`com.cshim.app`）、应用名「潮商会」、版本与权限文案等。
...
| **Signing**         | 在 Xcode 中为 Runner target 配置 Team、Bundle Identifier（如 com.cshim.app）、Provisioning Profile；推送需勾选 Push Notifications capability。 |
...
| **Bundle ID / 应用名** | Xcode 中 PRODUCT_BUNDLE_IDENTIFIER（如 com.cshim.app）；Info.plist 中 CFBundleName/CFBundleDisplayName「潮商会」。 |
