...
> IM 客户端在 iOS 端的原生宿主工程，承载 Flutter 引擎与「锦绣汇」应用在 iPhone / iPad 上的运行，负责推送、权限、后台模式、CallKit/Intent 等平台能力。
...
- **应用入口与配置**：`AppDelegate`、`Runner/Info.plist`、Bundle ID（`com.yixinim.app`）、应用名「锦绣汇」、版本与权限文案等。
...
| **Bundle ID / 应用名** | Xcode 中 PRODUCT_BUNDLE_IDENTIFIER（如 com.yixinim.app）；Info.plist 中 CFBundleName/CFBundleDisplayName「锦绣汇」。 |
