...
// SetCluster 注入集群桥接器（在 main.go 中初始化后调用）
func (h *Hub) SetCluster(cb *ClusterBridge) {
	h.cluster = cb
}
...
const (
	...
	defaultBroadcastWorkers = 64 // 广播消息处理协程数（需要更多，因为涉及遍历和发送）
)
...
func (h *Hub) handleRegister(client *Client) {
	...
	// ========== 新增：集群路由注册 ==========
	if h.cluster != nil {
		// 注册用户路由到 Redis（uid → 本节点）
		h.cluster.RegisterUser(userID, client.DeviceID)

		// 异步：加入群在线成员集合（避免阻塞注册流程）
		go func() {
			groupIDs := getUserGroupIDs(h.db, userID)
			h.cluster.JoinGroupOnline(userID, groupIDs)
		}()

		// 异步：拉取并下发离线消息
		go func() {
			msgs, err := h.cluster.GetOfflineMsgs(userID)
...
func (h *Hub) handleUnregister(client *Client) {
	...
	if isGoingOffline {
		...
		// ========== 新增：集群路由注销 ==========
		if h.cluster != nil {
			h.cluster.UnregisterUser(userID, client.DeviceID)
			go h.cluster.LeaveGroupOnline(userID)
		}
		// ========== 新增结束 ==========
	}
}
...
func (h *Hub) handleBroadcast(msg *BroadcastMessage) {
	...
	log.Printf("[WS Hub] Broadcasting: type=%s, userIDs_count=%d, chatID=%s",
		msg.Type, len(msg.UserIDs), msg.ChatID)
	...
	// 发送给特定用户（分片并行，避免大群串行遍历阻塞worker）
	if len(msg.UserIDs) > 0 {
		// 去重
		seen := make(map[string]struct{}, len(msg.UserIDs))
		uniq := make([]string, 0, len(msg.UserIDs))
		...
		// ≤200人直接串行，无需额外goroutine开销
		if len(uniq) <= 200 {
			for _, uid := range uniq {
				h.sendToUser(uid, data, msg.ExcludeID)
			}
		} else {
			// 大群：按256分片并行推送
			const shardSize = 256
			var wg sync.WaitGroup
			for i := 0; i < len(uniq); i += shardSize {
				endIdx := i + shardSize
				if endIdx > len(uniq) {
					endIdx = len(uniq)
				}
...
// sendToUserLocal 只推本节点内存中的连接（供 ClusterBridge 内部调用）
// ========== 新增 ==========
func (h *Hub) sendToUserLocal(userID string, data []byte) {
...
// SendToChatCluster 集群模式下向会话所有成员广播（跨节点路由）
func (h *Hub) SendToChatCluster(chatID string, data interface{}) {
	if h.cluster == nil {
		h.SendToChat(chatID, data, "")
		return
	}
	raw, err := json.Marshal(data)
	if err != nil {
		log.Printf("[WS Hub] SendToChatCluster marshal error: %v", err)
		return
	}
	h.cluster.BroadcastToGroup(chatID, raw)
}
...
// ========== 新增：集群模式对外入口，业务层统一调用这组方法 ==========

// SendToUserCluster 集群模式下向指定用户发消息（自动跨节点路由）
func (h *Hub) SendToUserCluster(userID string, data interface{}) {
	if h.cluster == nil {
		// 未启用集群，降级走本地
		h.SendToUser(userID, data)
		return
	...
	raw, err := json.Marshal(data)
	if err != nil {
		log.Printf("[WS Hub] SendToUserCluster marshal error: %v", err)
		return
	}
	h.cluster.SendToUser(userID, raw)
}

// SendToUsersCluster 集群模式下向多个用户发消息
func (h *Hub) SendToUsersCluster(userIDs []string, data interface{}) {
	if h.cluster == nil {
	...
	raw, err := json.Marshal(data)
	if err != nil {
		log.Printf("[WS Hub] SendToUsersCluster marshal error: %v", err)
		return
	}
	h.cluster.SendToUsers(userIDs, raw)
}

// BroadcastToGroupCluster 集群模式下向群推送（4万人群聊安全，分批并发）
func (h *Hub) BroadcastToGroupCluster(groupID string, data interface{}) {
	if h.cluster == nil {
		// 降级走本节点 chatSubscribers
		h.SendToChat(groupID, data, "")
		return
	}
	raw, err := json.Marshal(data)
	if err != nil {
		log.Printf("[WS Hub] BroadcastToGroupCluster marshal error: %v", err)
		return
	}
	h.cluster.BroadcastToGroup(groupID, raw)
}

// IsUserOnlineCluster 集群模式下检查用户是否在线（跨所有节点）
func (h *Hub) IsUserOnlineCluster(userID string) bool {
	if h.cluster == nil {
		return h.IsUserOnline(userID)
	}
	return h.cluster.IsUserOnlineCluster(userID)
}
...
// getUserGroupIDs 从数据库查询用户所在群组ID列表
// 用于用户上线时初始化群在线成员集合
// ========== 新增 ==========
func getUserGroupIDs(db *gorm.DB, userID string) []string {
	if db == nil {
		return nil
	}
	// chat_members 表，type=2 为群聊，查用户所在的所有群 chat uuid
	var chatIDs []string
	db.Table("chat_members").
		Select("chats.uuid").
		Joins("JOIN chats ON chats.id = chat_members.chat_id").
		Where("chat_members.user_id = (SELECT id FROM users WHERE uuid = ?) AND chats.type = 2", userID).
		Pluck("chats.uuid", &chatIDs)
	return chatIDs
}

// ========== 新增结束 ==========

// ClusterEnabled 返回当前 Hub 是否已启用集群模式
func (h *Hub) ClusterEnabled() bool {
	return h.cluster != nil
}

// SendToAllCluster 集群模式下向所有在线用户广播消息
// 集群模式：通过 Redis Pub/Sub 广播到所有节点，每个节点再本地推送
// 单机模式：降级走本地 SendToAll
func (h *Hub) SendToAllCluster(data interface{}) {
	if h.cluster == nil {
		h.SendToAll(data)
		return
	}
	raw, err := json.Marshal(data)
	if err != nil {
		log.Printf("[WS Hub] SendToAllCluster marshal error: %v", err)
		return
	}
	h.cluster.BroadcastToAll(raw)
}
