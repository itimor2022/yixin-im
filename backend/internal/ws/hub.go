...
func (cs *ClientSet) Range(f func(*Client)) {
	// 回调执行期间持有读锁；回调不得直接增删同一集合，避免锁升级导致死锁。
	cs.RLock()
	defer cs.RUnlock()
...
// Hub 默认工作协程数量 // 注册/注销 worker: 处理连接建立和断开 // 广播 worker: 处理消息推送
const (
...
func (h *Hub) Run() {
	...
	// 多个广播 worker 提高吞吐，但不承诺跨事件的全局处理顺序；
	// 消息顺序以持久化 seq 为准，客户端不能把 WS 到达顺序当作权威。
...
func (h *Hub) handleRegister(client *Client) {
	...
	atomic.StoreInt64(&h.lastConnectedUnix, time.Now().Unix())
...
func (h *Hub) handleUnregister(client *Client) {
	// Close 是幂等门闩，保证 readPump、writePump 等并发退出路径只做一次计数和集合清理。
	...
	nextOnline := atomic.AddInt64(&h.onlineCount, -1)
	if nextOnline < 0 {

		atomic.StoreInt64(&h.onlineCount, 0)
		nextOnline = 0
	}
	atomic.AddInt64(&h.totalDisconnects, 1)
	atomic.StoreInt64(&h.lastDisconnectedUnix, time.Now().Unix())
...
func (h *Hub) handleBroadcast(msg *BroadcastMessage) {
	...
	log.Printf("[WS Hub] Broadcasting: type=%s, target=%s, user_count=%d, chatID=%s, payload_bytes=%d",

		msg.Type, target, len(msg.UserIDs), msg.ChatID, len(data))
	if msg.All {
...
func (h *Hub) RecordDroppedMessage() {
	atomic.AddInt64(&h.droppedMessages, 1)
}
func wsUnixTimeString(value int64) string {
	if value <= 0 {
...
func (h *Hub) GetStats() map[string]interface{} {
	return map[string]interface{}{
		...
		"total_disconnects": atomic.LoadInt64(&h.totalDisconnects),

		"dropped_messages": atomic.LoadInt64(&h.droppedMessages),

		"last_connected_at": wsUnixTimeString(atomic.LoadInt64(&h.lastConnectedUnix)),

		"last_disconnected_at": wsUnixTimeString(atomic.LoadInt64(&h.lastDisconnectedUnix)),
