...
import (
	...
	"gorm.io/gorm"
	"gorm.io/gorm/clause" // WalletHandler 处理余额、红包、转账、充值和提现等资金操作。 // 余额与交易流水以 MySQL 事务结果为准，聊天消息和 WebSocket 只承担提交后的业务通知。
	"log"
	"math"
	"math/big"
	"net/http"
...
func (h *WalletHandler) sendChatMessageWithRetry(
	...
	// 钱包业务已经提交后才发送聊天消息；固定 MsgID 让一次重试命中消息幂等，
	// 通知失败不会重复扣款，也不会改变红包/转账的权威状态。
...
func walletPreviewText(msgType int, content map[string]interface{}) string {
	...
	case models.MsgTypeSystem:
		...
		if text, ok := content["text"].(string); ok {
			...
			return normalizeStoredChatPreviewText(text, models.MsgTypeSystem)
...
func NewWalletHandler(db *gorm.DB, cacheStore *cache.Cache, hub *ws.Hub, msgService *services.MessageService) *WalletHandler {
...
type redPacketAccessOperation int

const (
	redPacketAccessView redPacketAccessOperation = iota
	redPacketAccessClaim
)

type redPacketAccessContext struct {
	Chat     models.Chat
	IsSender bool
	IsMember bool
	Allowed  bool
...
func nextRedPacketClaimProgress(redPacket models.RedPacket, claimAmount float64) (float64, int, string) {
	remainingAmount := math.Round((redPacket.RemainingAmount-claimAmount)*100) / 100
	remainingCount := redPacket.RemainingCount - 1
	status := redPacket.Status
	if remainingCount == 0 {

		status = models.RedPacketStatusFinished
	}
	return remainingAmount, remainingCount, status
}
func decideRedPacketAccess(chat models.Chat, isSender, isMember bool, operation redPacketAccessOperation) redPacketAccessContext {
	access := redPacketAccessContext{
	...
	if operation == redPacketAccessView {

		access.Allowed = isSender || isMember

		return access
	}
	// 领取必须是当前会话成员。发送者即使仍可查看自己发出的红包，
	// 退群后也不能继续领取。
	if !isMember {

		if isSender {

			access.Status = http.StatusForbidden

			access.Message = "您已不在该会话中，无法领取红包"

		}

		return access
	}
	if chat.Type == 1 && isSender {

		access.Status = http.StatusForbidden

		access.Message = "不能领取自己发出的私聊红包"
	...
	if chat.Status == models.ChatStatusBanned {
		...
		access.Message = "该群组已被管理员封禁，无法领取红包"
...
func (h *WalletHandler) authorizeRedPacketAccess(
	db *gorm.DB,
	redPacket *models.RedPacket,
	userID uint64,
	operation redPacketAccessOperation) (redPacketAccessContext, error) {
	var chat models.Chat
	if err := db.Where("uuid = ?", redPacket.ChatID).First(&chat).Error; err != nil {

		if err == gorm.ErrRecordNotFound {

			return redPacketAccessContext{Status: http.StatusNotFound, Message: "红包不存在"}, nil

		}

		return redPacketAccessContext{}, err
	}
	...
	return decideRedPacketAccess(chat, redPacket.SenderID == userID, memberCount > 0, operation), nil
}
...
func (h *WalletHandler) SetPayPassword(c *gin.Context) {
	...
	if wallet.HasPayPassword() {
		...
		if !checkPayPasswordThrottle(c, h.cache, userID) {
		...
		if !wallet.CheckPayPassword(req.OldPassword) {
			...
			recordPayPasswordFailure(c, h.cache, userID)
		...
		clearPayPasswordFailures(c, h.cache, userID)
	}
	// 设置新密码
...
func (h *WalletHandler) Recharge(c *gin.Context) {
	response.Error(c, http.StatusGone, "直接充值接口已禁用，请使用充值订单或在线支付")
}
...
func (h *WalletHandler) SendRedPacket(c *gin.Context) {
	...
	// 钱包余额、红包本金和交易流水必须同事务提交；任一步失败都会回滚，
	// 防止出现“余额已扣但红包/流水缺失”的账务断裂。
	...
	if chat != nil && h.msgService != nil {
		...
		h.sendChatMessageWithRetry(context.Background(), params, sender.Nickname, sender.Avatar, sender.NicknameColor, sender.EmojiAvatar, targetUserIDs)
	}
	response.Success(c, gin.H{
...
func (h *WalletHandler) ClaimRedPacket(c *gin.Context) {
	...
	// 红包行是领取流程的并发闸门：锁内完成状态、剩余份数和重复领取检查，
	// 使多实例并发领取只能串行消耗同一份余额。
	// 使用行锁查找红包，防止并发问题
	...
	access, err := h.authorizeRedPacketAccess(tx, &redPacket, userID, redPacketAccessClaim)
	if err != nil {
	...
	if !access.Allowed {
		...
		response.Error(c, access.Status, access.Message)
	...
	if redPacket.Type == models.RedPacketTypeLucky {
		...
		// 拼手气红包 - 使用系统安全随机源按“分”生成随机金额。
		var err error

		claimAmount, err = h.calculateLuckyAmount(redPacket.RemainingAmount, redPacket.RemainingCount)

		if err != nil {

			tx.Rollback()

			log.Printf("[Wallet] failed to generate secure lucky amount: red_packet=%s err=%v", redPacket.UUID, err)

			response.Error(c, http.StatusInternalServerError, "红包金额计算失败，请重试")
	...
	// 更新红包
	remainingAmount, remainingCount, nextStatus := nextRedPacketClaimProgress(redPacket, claimAmount)
	...
	if nextStatus == models.RedPacketStatusFinished {
	...
	// 更新本地变量以便后续使用
	redPacket.RemainingAmount = remainingAmount
	redPacket.RemainingCount = remainingCount
	redPacket.Status = nextStatus
	// 创建领取记录
	...
	if redPacket.RemainingCount == 0 && redPacket.Type == models.RedPacketTypeLucky {
		...
		// 最终结算前先清理历史/异常标记，保证始终只有一条手气最佳记录。

		if err := tx.Model(&models.RedPacketClaim{}).
			Where("red_packet_id = ?", redPacket.ID).
	...
	// 红包进度、领取记录、收款余额和交易流水在同一事务内落库，
	// BalanceAfter 是审计快照，不能代替钱包行本身的权威余额。
	// 原子更新用户余额
	...
	if h.msgService != nil && redPacket.ChatID != "" {
		...
		if len(targetUserIDs) > 0 {
			...
			h.sendChatMessageWithRetry(context.Background(), params, claimer.Nickname, claimer.Avatar, claimer.NicknameColor, claimer.EmojiAvatar, targetUserIDs)
...
func (h *WalletHandler) GetRedPacket(c *gin.Context) {
	...
	access, err := h.authorizeRedPacketAccess(h.db, &redPacket, userID, redPacketAccessView)
	if err != nil {

		response.Error(c, http.StatusInternalServerError, "获取红包详情失败")
	...
	if err := h.db.Table("red_packet_claims AS rpc").
		Select(`rpc.id, 
...
COALESCE(NULLIF(users.nickname, ''), users.username, '') AS user_name, 


COALESCE(users.avatar, '') AS user_avatar, 
...
rpc.created_at`).
		Joins("LEFT JOIN users ON users.id = rpc.user_id").
		Where("rpc.red_packet_id = ?", redPacket.ID).
		Order("rpc.created_at ASC").
	...
	claimedTotalAmount := 0.0
	for i, item := range claims {

		claimedTotalAmount += item.Amount
	...
	claimedTotalAmount = math.Round(claimedTotalAmount*100) / 100
	response.Success(c, gin.H{
		...
		"claimed_total_amount": claimedTotalAmount,
...
func (h *WalletHandler) SendTransfer(c *gin.Context) {
	...
	// 转出时本金立即从可用余额扣除并进入 pending 转账；
	// 后续只有接收、拒收或过期退款中的一个状态分支能处置该笔本金。
	...
	if h.msgService != nil {
		...
		h.sendChatMessageWithRetry(context.Background(), params, sender.Nickname, sender.Avatar, sender.NicknameColor, sender.EmojiAvatar, []string{receiver.UUID})
	}
	response.Success(c, gin.H{
...
func (h *WalletHandler) AcceptTransfer(c *gin.Context) {
	...
	// 先锁转账行并在锁内检查 pending，再锁收款钱包；
	// 这是接收、拒收和过期任务共同遵守的状态机入口，避免本金被重复入账或退款。
...
`, sender.ID, receiver.ID).Row()
		...
		if row.Scan(&chatID) == nil && chatID > 0 {
			...
			if h.db.First(&chat, chatID).Error == nil {
				...
				h.sendChatMessageWithRetry(context.Background(), params, receiver.Nickname, receiver.Avatar, receiver.NicknameColor, receiver.EmojiAvatar, []string{sender.UUID, receiver.UUID})
...
func (h *WalletHandler) RejectTransfer(c *gin.Context) {
	...
	transferID := c.Param("id")
	// 拒收同样以转账行锁仲裁；状态改为 rejected、发送者退款和退款流水必须原子提交。
...
func (h *WalletHandler) CreateRechargeOrder(c *gin.Context) {
	...
	if err := h.db.Where("id = ? AND status = 1", req.MethodID).First(&method).Error; err != nil {

		response.Error(c, http.StatusNotFound, "充值方式不存在或已停用")
...
func (h *WalletHandler) CreateWithdrawRequest(c *gin.Context) {
	...
	if err := h.db.Where("id = ? AND status = 1", req.MethodID).First(&method).Error; err != nil {
		...
		response.Error(c, http.StatusNotFound, "提现方式不存在或已停用")
	...
	// 提现申请只移动可用余额到 frozen_balance，不在此处视为已出款；
	// 后台审核必须通过申请状态决定扣除或解冻，不能再次从可用余额扣款。
...
// 辅助函数  // getUserIDFromContext 从上下文获取用户ID（从UUID转换为数据库ID）
...
func (h *WalletHandler) calculateLuckyAmount(remaining float64, count int) (float64, error) {
	remainingCents := int64(math.Round(remaining * 100))
	amountCents, err := secureLuckyAmountCents(remainingCents, count)
	if err != nil {
...
// secureLuckyAmountCents 使用二倍均值法生成拼手气红包金额。 // 金额全程以“分”为单位，并使用 crypto/rand，避免浮点误差和可预测伪随机序列。
func secureLuckyAmountCents(remainingCents int64, count int) (int64, error) {
	if count <= 0 {

		return 0, fmt.Errorf("invalid remaining count: %d", count)
	}
	if remainingCents < int64(count) {

		return 0, fmt.Errorf("remaining amount %d cents is insufficient for %d claims", remainingCents, count)
	}
	if count == 1 {

		return remainingCents, nil
	}
	// 二倍均值法：上限随剩余金额和人数动态变化。
	maxCents := remainingCents * 2 / int64(count)
	// 至少为后续每个红包保留 1 分钱。
	maxAvailable := remainingCents - int64(count-1)
	if maxCents > maxAvailable {

		maxCents = maxAvailable
	}
	if maxCents < 1 {

		return 0, fmt.Errorf("invalid lucky amount upper bound: %d", maxCents)
	}
	randomOffset, err := rand.Int(rand.Reader, big.NewInt(maxCents))
	if err != nil {

		return 0, fmt.Errorf("generate secure random amount: %w", err)
	}
	return randomOffset.Int64() + 1, nil
}
