...
const (
	...
	maxMomentPageSize     = 50
)

type momentPageCursor struct {
	CreatedAt string `json:"t"`
	ID        uint64 `json:"i"`
}

func parseMomentPage(raw string) int {
...
func parseMomentPageSize(raw string) int {
	pageSize, err := strconv.Atoi(raw)
	if err != nil || pageSize < 1 {

		return defaultMomentPageSize
	}
	if pageSize > maxMomentPageSize {

		return maxMomentPageSize
	}
	return pageSize
}
func encodeMomentCursor(moment models.Moment) string {
	payload, err := json.Marshal(momentPageCursor{

		CreatedAt: moment.CreatedAt.UTC().Format(time.RFC3339Nano),
...
func decodeMomentCursor(raw string) (time.Time, uint64, error) {
	data, err := base64.RawURLEncoding.DecodeString(raw)
	if err != nil {

		return time.Time{}, 0, err
	}
	var cursor momentPageCursor
...
func (h *MomentHandler) findMomentPage(
	query *gorm.DB,
	pageSize int,
	cursor string,
	useCursor bool,
	offset int) ([]models.Moment, bool, string, error) {
	// 游标条件必须与 (created_at, id) 的稳定降序完全一致，避免同时间发布的动态重复或遗漏。
	q := query.Order("created_at DESC, id DESC")
	limit := pageSize
	if useCursor {

		if cursor != "" {

			createdAt, id, err := decodeMomentCursor(cursor)
	...
	hasMore := false
	if useCursor && len(moments) > pageSize {
	...
	if hasMore && len(moments) > 0 {
		...
		nextCursor = encodeMomentCursor(moments[len(moments)-1])
	}
	return moments, hasMore, nextCursor, nil
}
...
func (h *MomentHandler) GetMomentList(c *gin.Context) {
	...
	page := parseMomentPage(rawPage)
	pageSize := parseMomentPageSize(c.DefaultQuery("page_size", strconv.Itoa(defaultMomentPageSize)))
	cursor := c.Query("cursor")
	useCursor := cursor != "" || !hasPage
	...
	// 广场入口只暴露审核通过且公开的动态；用户屏蔽和平台合规条件在此基础上继续收窄结果集。
	...
	if compliance.Enabled && !compliance.MomentVideoEnabled && isIOSClientRequest(c, h.db, currentUID) {

		query = query.Where("content_type <> ?", models.ContentTypeVideo)
	}
	// 过滤屏蔽的动态和用户（如果用户已登录）
	if currentUID > 0 {
	...
	if topic != "" {
		...
		query = query.Where("JSON_CONTAINS(topics, ?)", jsonStringParam(topic))
	}
	// 按用户筛选
	if userID != "" {
	...
	moments, hasMore, nextCursor, err := h.findMomentPage(query, pageSize, cursor, useCursor, offset)
	if err != nil {

		if cursor != "" {

			response.BadRequest(c, "分页游标无效")
	...
	if !useCursor {
		...
		hasMore = int64(offset+len(moments)) < total
...
func (h *MomentHandler) GetMoment(c *gin.Context) {
	...
	compliance := loadIOSComplianceConfig(h.db)
	if moment.ContentType == models.ContentTypeVideo && compliance.Enabled &&

		!compliance.MomentVideoEnabled && isIOSClientRequest(c, h.db, 0) {
	...
	// 详情入口不能依赖列表过滤：直接访问 ID 时仍要独立校验审核状态和受众可见性。
...
func (h *MomentHandler) PublishMoment(c *gin.Context) {
	...
	compliance := loadIOSComplianceConfig(h.db)
	if req.ContentType == models.ContentTypeVideo && compliance.Enabled &&

		!compliance.MomentVideoEnabled && isIOSClientRequest(c, h.db, uid) {

		response.Error(c, http.StatusForbidden, "当前 iOS 版本暂不支持发布视频动态")

		return
	}
	// 先执行违禁词策略，再根据审核开关决定 normal/pending 状态；未审核内容不会进入公开列表。
	...
	if reviewEnabled {
		...
		// pending 状态只对作者和后台审核链路可见，公开广场始终只查询 normal。
...
func (h *MomentHandler) GetComments(c *gin.Context) {
	// 评论是动态的子资源；后续扩展此入口时必须与详情接口复用同一套状态和受众可见性校验。
	momentID := c.Param("id")
...
func (h *MomentHandler) GetMyMoments(c *gin.Context) {
	...
	compliance := loadIOSComplianceConfig(h.db)
	if compliance.Enabled && !compliance.MomentVideoEnabled && isIOSClientRequest(c, h.db, user.ID) {
...
func (h *MomentHandler) SearchMoments(c *gin.Context) {
	...
	var moments []models.Moment
	var total int64
	// 搜索与广场列表保持相同的公开边界，不能让待审核或非公开动态通过关键词绕过过滤。
	query := h.db.Model(&models.Moment{}).
		Where("status = ? AND visibility = ?", models.MomentStatusNormal, models.VisibilityPublic).
		Where("content LIKE ? OR JSON_CONTAINS(topics, ?)", "%"+keyword+"%", jsonStringParam(keyword))
	compliance := loadIOSComplianceConfig(h.db)
	if compliance.Enabled && !compliance.MomentVideoEnabled && isIOSClientRequest(c, h.db, 0) {
