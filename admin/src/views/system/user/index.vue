...
<template>
  <div class="user-page art-full-height">
      ...
      <ArtTableHeader v-model:columns="columnChecks" :loading="loading" @refresh="handleRefresh">
        <template #left>
          <div class="flex items-center gap-4">
            ...
            <ElDivider direction="vertical" />
            <ElButton type="primary" @click="openExportDialog"> 导出下级 </ElButton>
          </div>
        </template>
      </ArtTableHeader>
    ...
    <!-- 导出下级用户弹窗：输入根用户ID，递归拉取所有下级并下载为 Excel -->
    <ElDialog
    ...
    >
      <ElForm :model="exportForm" label-width="100px" @submit.prevent="submitExport">
        <ElFormItem label="用户 ID" required>
          <ElInput
            v-model="exportForm.rootId"
            placeholder="请输入用户 ID（表格里 ID 列的值）"
            maxlength="20"
            clearable
        ...
        <ElFormItem>
          <div class="text-xs text-g-400 leading-5">
            <p>· 系统会从该用户出发，沿「推荐人」关系逐层展开下级，最多 32 层 / 10000 个节点。</p>
            <p>· 导出字段：用户名、昵称、手机号、最后登录 IP（最近一次活跃设备的 IP）。</p>
            <p>· 列表中不包含根用户本身。</p>
          </div>
        </ElFormItem>
      </ElForm>
      <template #footer>
        ...
        <ElButton type="primary" :loading="exporting" @click="submitExport"> 查询并导出 </ElButton>
      </template>
    </ElDialog>
  </div>
</template>
...
<script setup lang="ts">
      ...
      columnsFactory: () => [
            ...
            return h('div', { class: 'user-info-cell flex items-center py-1' }, [
              ...
              h('div', { class: 'ml-3 min-w-0 flex-1' }, [
                ...
                h('div', { class: 'mt-1 flex items-center gap-2' }, [
                  h(
                    'span',
                    { class: 'rounded bg-gray-50 px-1.5 py-0.5 font-mono text-xs text-orange-400' },
        ...
        },
        {
          ...
          formatter: (row) => {
            ...
            return h('span', { class: 'font-mono text-sm text-g-700' }, row.phone)
          }
        },
        {
          prop: 'inviteCode',
          ...
          formatter: (row) => {
            ...
            return h('span', { class: 'invite-code-text' }, row.inviteCode)
          }
        },
        {
          prop: 'recommender',
          ...
          formatter: (row) => {
            ...
            const name =
              row.recommenderNickname || row.recommenderUsername || `ID:${row.recommenderId}`
            const code = row.recommenderInviteCode ? `邀请码：${row.recommenderInviteCode}` : ''
            return h(
              'div',
              {
                class: 'recommender-cell',
                style: 'line-height: 1.3; padding: 2px 0;' // 紧凑显示
              },
              [
                h(
                  'div',
                  {
                    class: 'recommender-name text-orange-400',
                    style: 'font-size: 13px;' // 减小字号
                  },
                  `${name}`
                ),
                code
                  ? h(
                      'div',
                      {
                        class: 'recommender-code text-g-500',
                        style: 'font-size: 11px; color: #999;' // 更小的字号
                      },
                      code
                    )
                  : null
          ...
          formatter: (row) => {
            ...
            if (displayName) {
              return h('div', { class: 'bind-id-cell' }, [
                ...
                h('div', { class: 'text-xs text-orange-400 font-mono' }, `ID:${row.bindId}`)
              ])
            }
  ...
  /**
   * 把查询到的下级列表生成 Excel 并触发浏览器下载。
   * 复用项目里已有的 write-excel-file（按需 import，不进入主包），与 art-excel-export 行为一致。
   */
  const writeSubordinatesToExcel = async (
    rootInfo: { id: number; username: string; nickname: string },
    rows: Array<{
      id: number
      username: string
      nickname: string
    ...
    // 表头顺序与 Excel 列顺序一致，便于阅读
    const headerRow = [
      { value: '用户ID', type: String, fontWeight: 'bold' as const },
      { value: '用户名', type: String, fontWeight: 'bold' as const },
      { value: '昵称', type: String, fontWeight: 'bold' as const },
      { value: '手机号', type: String, fontWeight: 'bold' as const },
      { value: '最后登录IP', type: String, fontWeight: 'bold' as const }
    ]

    // 借用 art-excel-export 同样的 SheetData 类型，保持与项目其它导出行为一致。
    type SheetData = import('write-excel-file/browser').SheetData
    const sheetData: SheetData = [headerRow as unknown as SheetData[number]]
    for (const r of rows) {
      sheetData.push([
        { value: r.id, type: Number },
        { value: r.username ?? '', type: String },
        { value: r.nickname ?? '', type: String },
        { value: r.phone ?? '', type: String },
        { value: r.device_ip ?? '', type: String }
      ] as unknown as SheetData[number])
    }

    const { default: writeXlsxFile } = await import('write-excel-file/browser')

    const safeNick = (rootInfo.nickname || rootInfo.username || `user_${rootInfo.id}`).replace(
      /[\\/:*?"<>|]/g,
      '_'
    )
    const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19)
    const filename = `subordinates_${rootInfo.id}_${safeNick}_${stamp}.xlsx`

    const excelFile = writeXlsxFile(sheetData, {
      sheet: '下级用户',
      columns: [{ width: 12 }, { width: 20 }, { width: 22 }, { width: 16 }, { width: 18 }]
    })
    await excelFile.toFile(filename)
  }
   ...
   *   1) 校验输入；
   *   2) 调用后端递归查询接口；
   *   3) 用 write-excel-file/browser 生成 xlsx 并触发下载。
   */
  const submitExport = async (): Promise<void> => {
    const raw = exportForm.rootId.trim()
    if (raw === '') {
    ...
    try {
      // 1) 拉数据：耗时操作统一在 loading 期间提示
      const loadingMsg = ElMessage({
        message: '正在查询所有下级，请稍候…',
        type: 'info',
        duration: 0
      })
      let resp
      try {
        resp = await fetchExportSubordinates(rootId)
      } finally {
        loadingMsg.close()
      }

      if (!resp || !Array.isArray(resp.list)) {
        ElMessage.error('查询结果格式异常')
        return
      }

      if (resp.list.length === 0) {
        ElMessage.warning(`用户 ${rootId} 下没有任何下级，未生成 Excel`)
        return
      }

      // 2) 生成 Excel 并下载
      try {
        await writeSubordinatesToExcel(resp.root_user, resp.list)
      } catch (err) {
        console.error('生成 Excel 失败:', err)
        ElMessage.error('生成 Excel 失败')
        return
      }

      const suffix = resp.truncated ? '（已截断到上限，可能不完整）' : ''
      ElMessage.success(`已导出 ${resp.list.length} 条下级用户${suffix}`)
      exportDialogVisible.value = false
    } catch (error) {
      console.error('导出下级失败:', error)
      // http 拦截器已经提示过错误，这里避免重复 toast
    } finally {
