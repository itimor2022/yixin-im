    ...
    <ElCard class="art-table-card" shadow="never">
      ...
      <ElDialog v-model="vipDialogVisible" title="开通/续期会员" width="500px" destroy-on-close>
        <ElForm :model="vipGrantForm" label-width="96px">
          ...
          <ElFormItem label="套餐" required>
            <ElSelect v-model="vipGrantForm.plan_id" placeholder="选择会员套餐" class="w-full" filterable
              :loading="vipPlansLoading" @change="syncVipGrantDays">
              <ElOption v-for="plan in vipPlans" :key="plan.id" :label="`${plan.name} · ${plan.duration_days}天`"
                :value="plan.id" />
            </ElSelect>
          </ElFormItem>
          <ElFormItem label="有效期">
            <ElInputNumber v-model="vipGrantForm.days" :min="1" :max="3650" controls-position="right" />
      ...
      <ElDialog v-model="diagnosticVisible" title="用户诊断" width="920px" destroy-on-close>
        <div v-if="diagnosticData" class="diagnostic-panel">
          <div class="diagnostic-grid">
            ...
            <ElCard shadow="never">
              ...
              <div class="diagnostic-checks">
                <ElAlert v-for="item in diagnosticData.checks" :key="item.title" :title="item.title"
                  :description="item.detail" :type="checkAlertType(item.level)" :closable="false" show-icon />
              </div>
            </ElCard>
          </div>
          ...
          <ElCard shadow="never" class="mt-4">
            ...
            <ElTable :data="diagnosticData.recent_chats" size="small" border>
              ...
              <ElTableColumn prop="last_msg_text" label="最后消息" min-width="220" show-overflow-tooltip />
        ...
        <template #footer>
          ...
          <ElButton type="primary" :loading="testPushLoading"
            :disabled="!diagnosticData || diagnosticData.summary.push_bound_count === 0" @click="handleTestPush">
            发送测试推送
          </ElButton>
        </template>
      </ElDialog>
