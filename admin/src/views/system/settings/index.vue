  ...
  <div class="settings-page">
    <ElTabs v-model="activeTab" type="border-card">
      ...
      <ElTabPane label="基本信息" name="basic">
        ...
        <ElForm ref="basicFormRef" :model="basicForm" :rules="basicFormRules" label-width="140px" class="max-w-2xl">
          <ElFormItem label="系统名称" prop="system_name">
            <ElInput v-model="basicForm.system_name" maxlength="50" show-word-limit placeholder="如：My Admin"
              :disabled="isDemoAdmin" />
            <span class="mt-1 text-xs text-g-400">显示在左侧菜单顶部，留空则使用默认名称，最多 50 个字符</span>
          </ElFormItem>
          <ElFormItem label="系统版本" prop="system_version">
            <ElInput v-model="basicForm.system_version" placeholder="如：1.0.0" :disabled="isDemoAdmin" />
            <div class="mt-1 flex items-center gap-3 text-xs text-g-400">
              <span>客户端“关于”页将直接显示这里配置的系统版本，格式建议：1.0.0 或 1.0.0-beta.1</span>
              <ElButton v-if="!isDemoAdmin" link type="primary" size="small" @click="syncSystemVersionToPlatforms">
                同步到 iOS / Android 版本号
              </ElButton>
            </div>
          </ElFormItem>
          <ElFormItem label="邀请注册链接域名" prop="register_base_url">
            <ElInput v-model="basicForm.register_base_url" placeholder="如：https://app.example.com"
              :disabled="isDemoAdmin" />
            <span class="mt-1 text-xs text-g-400">客户端“邀请朋友”链接会使用该域名；留空则使用后端配置文件中的注册页域名</span>
          </ElFormItem>
          <ElDivider content-position="left">App版本</ElDivider>
          <ElFormItem label="iOS版本号" prop="app_version_ios">
            <ElInput v-model="basicForm.app_version_ios" placeholder="如：1.0.0" :disabled="isDemoAdmin" />
            <span class="mt-1 text-xs text-g-400">留空时保存将自动使用“系统版本”，格式如 1.0.0 或 1.0.0-beta.1</span>
          </ElFormItem>
          <ElFormItem label="Android版本号" prop="app_version_android">
            <ElInput v-model="basicForm.app_version_android" placeholder="如：1.0.0" :disabled="isDemoAdmin" />
            <span class="mt-1 text-xs text-g-400">留空时保存将自动使用“系统版本”，格式如 1.0.0 或 1.0.0-beta.1</span>
          </ElFormItem>
          ...
          <ElFormItem label="更新下载链接" prop="app_update_url">
            <ElInput v-model="basicForm.app_update_url" placeholder="App Store / 下载页面链接" :disabled="isDemoAdmin" />
            <span class="mt-1 text-xs text-g-400">开启强制更新时该项必填；请使用完整链接，如 https://example.com/app</span>
          </ElFormItem>
          <ElFormItem label="更新提示信息" prop="app_update_message">
            <ElInput v-model="basicForm.app_update_message" type="textarea" :rows="3" maxlength="500" show-word-limit
              placeholder="更新内容说明" :disabled="isDemoAdmin" />
      ...
      <ElTabPane label="功能设置" name="features">
        ...
        <ElForm :model="featureForm" label-width="180px" class="max-w-2xl">
          ...
          <ElFormItem label="强制绑定手机号">
            ...
            <span class="ml-2 text-sm text-g-400">开启后用户登录后必须先绑定手机号，未绑定前不可使用消息、通讯录、钱包等功能</span>
          </ElFormItem>
          ...
          <ElFormItem label="动态发布审核">
            ...
            <span class="ml-2 text-sm text-g-400">开启后用户发布动态需后台审核通过后才会在广场展示</span>
          </ElFormItem>
          ...
          <ElFormItem label="启用自定义栏目">
            ...
            <span class="ml-2 text-sm text-g-400">开启后，会在客户端“联系人”和“发现”之间显示一个可自定义的网站入口</span>
          </ElFormItem>
          <ElFormItem label="栏目名称">
            <ElInput v-model="featureForm.custom_portal_title" maxlength="20" show-word-limit placeholder="例如：官网"
              :disabled="!featureForm.custom_portal_enabled || isDemoAdmin" />
          </ElFormItem>
          <ElFormItem label="栏目图标">
            <div class="flex items-center gap-3">
              <ElUpload :action="portalUploadUrl" :headers="portalUploadHeaders" :show-file-list="false"
                accept="image/*" :disabled="!featureForm.custom_portal_enabled || isDemoAdmin"
                ...
                <div
                  ...
                  <img v-if="featureForm.custom_portal_icon_url" :src="fixImageUrl(featureForm.custom_portal_icon_url)"
                    alt="portal-icon" class="h-full w-full object-cover" />
          ...
          <ElFormItem label="打开网址">
            <ElInput v-model="featureForm.custom_portal_url" placeholder="例如：https://example.com"
              :disabled="!featureForm.custom_portal_enabled || isDemoAdmin" />
          ...
          <ElFormItem label="在线客服地址">
            <ElInput v-model="featureForm.customer_service_url"
              placeholder="例如：https://kefu.example.com 或 https://t.me/xxx" :disabled="isDemoAdmin" />
            <span class="ml-2 text-sm text-g-400">App「找回密码」与「常见问题-在线客服」会跳转此地址</span>
          </ElFormItem>
          <ElFormItem label="登录页面图片">
            <ElInput v-model="featureForm.logo_image_url" placeholder="例如：https://www.demo.com/1.jpg"
              :disabled="isDemoAdmin" />
          </ElFormItem>
          <ElFormItem label="发现页顶部图片">
            <ElInput v-model="featureForm.discover_top_image_url"
              placeholder="例如：https://www.demo.com/discover_banner.jpg" :disabled="isDemoAdmin" />
          </ElFormItem>
          ...
          <ElFormItem label="邀请码只加绑定客服">
            ...
            <span class="ml-2 text-sm text-g-400">开启后，邀请码注册用户只自动添加该邀请码绑定客服</span>
          </ElFormItem>
          ...
          <ElFormItem label="钱包功能">
            <ElSwitch v-model="featureForm.wallet_enabled" :disabled="isDemoAdmin" />
            <span class="ml-2 text-sm text-g-400">开启后"我的"页显示钱包入口</span>
          </ElFormItem>
          ...
          <ElFormItem label="群组成员上限">
            <ElInputNumber v-model="featureForm.group_max_members" :min="100" :max="1000000" :step="10000"
              controls-position="right" :disabled="isDemoAdmin" />
            <span class="ml-2 text-sm text-g-400">默认200000人</span>
          </ElFormItem>
          <ElFormItem label="频道订阅者上限">
            <ElInputNumber v-model="featureForm.channel_max_members" :min="0" :max="10000000" :step="100000"
              controls-position="right" :disabled="isDemoAdmin" />
            <span class="ml-2 text-sm text-g-400">0表示无限制</span>
          </ElFormItem>
          <ElFormItem label="消息撤回时限">
            <ElInputNumber v-model="featureForm.revoke_message_minutes" :min="1" :max="60" :step="1"
              controls-position="right" :disabled="isDemoAdmin" />
            <span class="ml-2 text-sm text-g-400">分钟，默认2分钟</span>
          </ElFormItem>
          <ElFormItem label="IP发送消息频率限制">
            <ElInputNumber v-model="featureForm.ip_rate_limit" :min="0" :max="10000" :step="10"
              controls-position="right" :disabled="isDemoAdmin" />
            <span class="ml-2 text-sm text-g-400">条/分钟（同一IP），0 表示不限制，默认60</span>
          </ElFormItem>
          <ElFormItem label="用户发消息频率限制">
            <ElInputNumber v-model="featureForm.user_rate_limit" :min="0" :max="10000" :step="10"
              controls-position="right" :disabled="isDemoAdmin" />
          ...
          <ElFormItem label="心跳超时时间">
            <ElInputNumber v-model="featureForm.heartbeat_timeout" :min="10" :max="600" :step="10"
              controls-position="right" :disabled="isDemoAdmin" />
            <span class="ml-2 text-sm text-g-400">秒，超过此时间未收到心跳则视为离线（默认60秒，修改后需重启后端）</span>
          </ElFormItem>
          ...
          <ElFormItem label="App ID">
            <ElInput v-model="featureForm.agora_app_id" :placeholder="isDemoAdmin ? '******' : '声网控制台获取的 App ID'"
              :disabled="!featureForm.agora_enabled || isDemoAdmin" />
          </ElFormItem>
          <ElFormItem label="App Certificate">
            <ElInput v-model="featureForm.agora_app_certificate"
              :placeholder="isDemoAdmin ? '******' : '声网控制台获取的 App Certificate'" type="password"
              :show-password="!isDemoAdmin" :disabled="!featureForm.agora_enabled || isDemoAdmin" />
          </ElFormItem>
          <ElFormItem label="Token 有效期">
            <ElInputNumber v-model="featureForm.agora_token_expire" :min="600" :max="86400" :step="600"
              controls-position="right" :disabled="!featureForm.agora_enabled || isDemoAdmin" />
          ...
          <ElFormItem label="Bundle ID">
            <ElInput v-model="featureForm.apns_bundle_id" :placeholder="isDemoAdmin ? '******' : '如：com.example.app'"
              :disabled="!featureForm.apns_enabled || isDemoAdmin" />
          </ElFormItem>
          <ElFormItem label="Key ID">
            <ElInput v-model="featureForm.apns_key_id" :placeholder="isDemoAdmin ? '******' : '10位 Key ID'"
              :disabled="!featureForm.apns_enabled || isDemoAdmin" maxlength="10" />
          </ElFormItem>
          <ElFormItem label="Team ID">
            <ElInput v-model="featureForm.apns_team_id" :placeholder="isDemoAdmin ? '******' : '10位 Team ID'"
              :disabled="!featureForm.apns_enabled || isDemoAdmin" maxlength="10" />
          </ElFormItem>
          <ElFormItem label="Auth Key (.p8)">
            <ElInput v-model="featureForm.apns_auth_key" type="textarea" :rows="4" :placeholder="isDemoAdmin
                ? '******（演示管理员无法查看）'
                : '粘贴 .p8 文件的完整内容（包含 -----BEGIN PRIVATE KEY-----）'
              " :disabled="!featureForm.apns_enabled || isDemoAdmin" />
          </ElFormItem>
          <ElFormItem label="环境">
            <ElRadioGroup v-model="featureForm.apns_environment" :disabled="!featureForm.apns_enabled || isDemoAdmin">
          ...
          <ElFormItem label="FCM Project ID">
            <ElInput v-model="featureForm.fcm_project_id" :placeholder="isDemoAdmin ? '******' : 'Firebase Project ID'"
              :disabled="!featureForm.fcm_enabled || isDemoAdmin" />
          </ElFormItem>
          <ElFormItem label="FCM Service Account JSON">
            <ElInput v-model="featureForm.fcm_service_account_json" type="textarea" :rows="4" :placeholder="isDemoAdmin ? '******（演示管理员无法查看）' : '粘贴 Firebase service account JSON'
              " :disabled="!featureForm.fcm_enabled || isDemoAdmin" />
          </ElFormItem>
          ...
          <ElFormItem label="HMS App ID">
            <ElInput v-model="featureForm.hms_app_id" :placeholder="isDemoAdmin ? '******' : '华为推送 App ID'"
              :disabled="!featureForm.hms_enabled || isDemoAdmin" />
          </ElFormItem>
          <ElFormItem label="HMS App Secret">
            <ElInput v-model="featureForm.hms_app_secret" type="password" :show-password="!isDemoAdmin"
          ...
          <ElFormItem label="小米包名">
            <ElInput v-model="featureForm.xiaomi_package_name"
          ...
          <ElFormItem label="小米 App Secret">
            <ElInput v-model="featureForm.xiaomi_app_secret" type="password" :show-password="!isDemoAdmin"
          ...
          <ElFormItem label="OPPO App Key">
            <ElInput v-model="featureForm.oppo_app_key" :placeholder="isDemoAdmin ? '******' : 'OPPO 推送 App Key'"
              :disabled="!featureForm.oppo_push_enabled || isDemoAdmin" />
          </ElFormItem>
          <ElFormItem label="OPPO App Secret">
            <ElInput v-model="featureForm.oppo_app_secret" type="password" :show-password="!isDemoAdmin"
              :placeholder="isDemoAdmin ? '******' : 'OPPO 推送 App Secret'"
              :disabled="!featureForm.oppo_push_enabled || isDemoAdmin" />
          </ElFormItem>
          <ElDivider content-position="left">文件上传限制</ElDivider>
          <ElFormItem label="图片大小限制">
            <ElInputNumber v-model="featureForm.max_image_size" :min="1" :max="100" :step="1" controls-position="right"
              :disabled="isDemoAdmin" />
            <span class="ml-2 text-sm text-g-400">MB，默认10MB</span>
          </ElFormItem>
          <ElFormItem label="视频大小限制">
            <ElInputNumber v-model="featureForm.max_video_size" :min="10" :max="1000" :step="10"
              controls-position="right" :disabled="isDemoAdmin" />
            <span class="ml-2 text-sm text-g-400">MB，默认100MB</span>
          </ElFormItem>
          <ElFormItem label="文件大小限制">
            <ElInputNumber v-model="featureForm.max_file_size" :min="10" :max="1000" :step="10"
              controls-position="right" :disabled="isDemoAdmin" />
            <span class="ml-2 text-sm text-g-400">MB，默认100MB</span>
          </ElFormItem>
          <ElFormItem label="语音大小限制">
            <ElInputNumber v-model="featureForm.max_voice_size" :min="1" :max="100" :step="1" controls-position="right"
              :disabled="isDemoAdmin" />
          ...
          <ElTableColumn label="群组" min-width="200">
            <template #default="{ row }">
              <div class="flex items-center gap-2">
                <ElAvatar :size="36" :src="getAvatarUrl(row.avatar, row.chat_uuid)" shape="square" />
                <div>
      ...
      <!-- 等级字段 -->
      <ElTabPane label="等级字段" name="levels">
        <div class="mb-4">
          <ElAlert v-if="isDemoAdmin" type="warning" :closable="false" show-icon>
            您当前是演示管理员，只能查看等级配置
          </ElAlert>
          <ElAlert v-else type="info" :closable="false" show-icon>
            用户等级和群组等级支持 Markdown 格式，将在 App 中展示等级说明
          </ElAlert>
        </div>
        <ElForm label-width="120px" class="max-w-4xl">
          <ElFormItem label="用户等级">
            <ElInput v-model="levelForm.user_levels" type="textarea" :rows="15" placeholder="请输入用户等级说明（支持 Markdown 格式）"
              :disabled="isDemoAdmin" />
            <span class="mt-1 text-xs text-g-400">定义用户等级体系，如：Lv.1 新手|Lv.2 进阶|Lv.3 专家等，用|隔开</span>
          </ElFormItem>
          <ElFormItem v-if="!isDemoAdmin">
            <ElButton type="primary" :loading="savingLevel" @click="saveUserLevels">
              保存用户等级
            </ElButton>
            <ElButton @click="previewLevel('user')">预览</ElButton>
          </ElFormItem>
          <ElFormItem v-else>
            <ElButton @click="previewLevel('user')">预览用户等级</ElButton>
          </ElFormItem>
          <ElDivider />
          <ElFormItem label="群组等级">
            <ElInput v-model="levelForm.group_levels" type="textarea" :rows="15" placeholder="请输入群组等级说明（支持 Markdown 格式）"
              :disabled="isDemoAdmin" />
            <span class="mt-1 text-xs text-g-400">定义群组等级体系，如：普通群|高级群|VIP群等，用|隔开</span>
          </ElFormItem>
          <ElFormItem v-if="!isDemoAdmin">
            <ElButton type="primary" :loading="savingLevel" @click="saveGroupLevels">
              保存群组等级
            </ElButton>
            <ElButton @click="previewLevel('group')">预览</ElButton>
          </ElFormItem>
          <ElFormItem v-else>
            <ElButton @click="previewLevel('group')">预览群组等级</ElButton>
          </ElFormItem>
        </ElForm>
      </ElTabPane>
      ...
      <ElTabPane label="协议文档" name="agreements">
        ...
        <ElForm label-width="120px" class="max-w-4xl">
          <ElFormItem label="用户协议">
            <ElInput v-model="agreementForm.user_agreement" type="textarea" :rows="15"
              placeholder="请输入用户协议内容（支持 Markdown 格式）" :disabled="isDemoAdmin" />
          </ElFormItem>
          <ElFormItem v-if="!isDemoAdmin">
          ...
          <ElFormItem label="隐私政策">
            <ElInput v-model="agreementForm.privacy_policy" type="textarea" :rows="15"
              placeholder="请输入隐私政策内容（支持 Markdown 格式）" :disabled="isDemoAdmin" />
          </ElFormItem>
          <ElFormItem v-if="!isDemoAdmin">
...
const loadSettings = async () => {
  try {
    ...
    // 等级字段
    levelForm.user_levels = settings.user_levels || ''
    levelForm.group_levels = settings.group_levels || ''
    // 协议文档
...
// 保存用户等级
const saveUserLevels = async () => {
  savingLevel.value = true
  try {
    await updateSystemSettings({ user_levels: levelForm.user_levels })
    ElMessage.success('用户等级已保存')
  } catch {
    ElMessage.error('保存失败')
  } finally {
...
// 保存群组等级
const saveGroupLevels = async () => {
  savingLevel.value = true
  try {
    await updateSystemSettings({ group_levels: levelForm.group_levels })
    ElMessage.success('群组等级已保存')
  } catch {
    ElMessage.error('保存失败')
  } finally {
    savingLevel.value = false
...
// 预览等级
const previewLevel = (type: 'user' | 'group') => {
  const content = type === 'user' ? levelForm.user_levels : levelForm.group_levels
  previewTitle.value = type === 'user' ? '用户等级预览' : '群组等级预览'
