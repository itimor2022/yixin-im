 ...
 * 等待路由初始化完成（用于并发导航场景）
 */
async function waitForRouteInitComplete(timeoutMs = 10000): Promise<boolean> {
  const start = Date.now()
  while (routeInitInProgress && Date.now() - start < timeoutMs) {
...
async function handleRouteGuard(
  ...
  if (!routeRegistry?.isRegistered() && userStore.isLogin) {
    ...
    if (routeInitInProgress) {
      // 正在初始化中：等待初始化完成后自动重试当前导航，避免页面空白
      const completed = await waitForRouteInitComplete()
      if (!completed || routeInitFailed || !routeRegistry?.isRegistered()) {
        next({ name: 'Exception500', replace: true })
        return
      }
      next({
