    ...
    <div class="w-full overflow-hidden" ref="scrollRef">
      ...
      >
        <li
          ...
          :class="[
            item.tabId === activeTab ? 'activ-tab !text-theme' : 'text-g-600 dark:text-g-800',
          ...
          :id="`scroll-li-${index}`"
          @click="clickTab(item)"
          @contextmenu.prevent="(e: MouseEvent) => showMenu(e, item.tabId)"
        >
          <ArtSvgIcon
            v-show="item.icon"
            :icon="item.icon"
            class="text-base mr-1 group-hover:text-theme"
            :class="item.tabId === activeTab ? 'text-theme' : 'text-g-600'"
          />
          ...
          <span
            ...
            @click.stop="closeWorktab('current', item.tabId)"
          >
...
<script setup lang="ts">
  ...
  const useContextMenu = () => {
    const getClickedTabInfo = () => {
      const clickedIndex = list.value.findIndex((tab) => tab.tabId === clickedTabId.value)
      ...
      return {
        ...
        isCurrentTab: clickedTabId.value === activeTab.value
      }
    }
  ...
  const useTabOperations = (adjustPositionAfterClose: () => void) => {
    ...
    const closeWorktab = (type: TabCloseType, tabId?: string) => {
      ...
      const closeActions = {
        current: () => store.removeTab(targetTabId),
        left: () => store.removeLeft(targetTabId),
        right: () => store.removeRight(targetTabId),
        other: () => store.removeOthers(targetTabId),
        all: () => store.removeAll()
      }
    ...
    const showMenu = (e: MouseEvent, tabId?: string) => {
      clickedTabId.value = tabId || ''
    ...
    const handleSelect = (item: MenuItemType) => {
      ...
      if (key === 'fixed') {
        useWorktabStore().toggleFixedTab(clickedTabId.value)
        return
      }

      const activeIndex = list.value.findIndex((tab) => tab.tabId === activeTab.value)
      const clickedIndex = list.value.findIndex((tab) => tab.tabId === clickedTabId.value)
      ...
      if (shouldNavigate) {
        const clickedTab = list.value.find((tab) => tab.tabId === clickedTabId.value)
        if (clickedTab) {
          router.push({ path: clickedTab.path, query: clickedTab.query as LocationQueryRaw })
        }
      }

      closeWorktab(key as TabCloseType, clickedTabId.value)
    }
