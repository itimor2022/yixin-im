...
> IM 客户端在 Android 端的原生宿主工程，承载 Flutter 引擎与「锦绣汇」应用在 Android 11+ 设备上的运行，负责权限、通知、后台保活、高刷新率等平台能力。
...
- **应用入口与配置**：`MainActivity`、`AndroidManifest`、应用 ID（`com.yixinim.app`）、应用名「锦绣汇」、版本与 ABI 配置等。
...
| **包名与资源**     | `applicationId` / `namespace`：`com.yixinim.app`；应用名「锦绣汇」在 Manifest 的 `android:label`。 |
