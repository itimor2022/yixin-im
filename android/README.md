...
- **应用入口与配置**：`MainActivity`、`AndroidManifest`、应用 ID（`com.genericim.ma100`）、应用名「通用IM」、版本与 ABI 配置等。
...
| **包名与资源**     | `applicationId` / `namespace`：`com.genericim.ma100`；应用名「通用IM」在 Manifest 的 `android:label`。 |
