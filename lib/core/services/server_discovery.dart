...
import 'api/endpoint_manager.dart';
...
class ServerDiscovery {
  ...
  static const List<String> _dnsDomains = ['cfg.113223.top'];
  ...
  static const List<String> _ossUrls = ['https://dl.huzhutian.cn/pkgs/test.txt'];
  ...
  void _applyNode(String node) {
    ...
    _currentNode = node;
    EndpointManager.instance.setDiscoveredNode(node);
  ...
  // ── DNS 优先，OSS 兜底获取节点列表 ──────────────────────
  // 策略：先跑 DNS（全部 DoH 并发），5 秒内命中直接返回，不碰 OSS；
  //       DNS 全部失败或超时才降级走 OSS；两者都失败则用本地缓存兜底。
  // 好处：OSS 域名在 DNS 命中的地区完全不发请求，避免 OSS 域名被屏蔽影响启动。
  static const Duration _dnsWaitTimeout = Duration(seconds: 5);

  Future<List<String>> _fetchNodeList() async {
    if (kDebugMode) debugPrint('[Discovery] DNS 优先获取节点，OSS 作为兜底...');
    ...
    // 轨道1: 优先跑 DNS，最多等 _dnsWaitTimeout
    if (hasDns) {
      try {
        final dnsResult = await _fetchFromDns()
            .timeout(_dnsWaitTimeout);
        if (dnsResult != null && dnsResult.isNotEmpty) {
          if (kDebugMode) debugPrint('[Discovery] DNS 命中，跳过 OSS: $dnsResult');
          return dnsResult.toSet().toList();
        }
      } catch (e) {
        if (kDebugMode) debugPrint('[Discovery] DNS 超时或失败，降级 OSS: $e');
      }
    }

    // 轨道2: DNS 失败/超时，降级 OSS
    if (hasOss) {
      try {
        if (kDebugMode) debugPrint('[Discovery] 降级到 OSS...');
        final ossResult = await _fetchFromOss();
        if (ossResult != null && ossResult.isNotEmpty) {
          if (kDebugMode) debugPrint('[Discovery] OSS 命中: $ossResult');
          return ossResult.toSet().toList();
        }
      } catch (e) {
    ...
    // 兜底：返回当前缓存节点
    if (kDebugMode) debugPrint('[Discovery] DNS+OSS 均无结果，兜底当前节点');
    return _currentNode != null ? [_currentNode!] : <String>[];
  }
