<#
.SYNOPSIS
快速验证 Android 真机上的资料、设置和消息写入流程。

.DESCRIPTION
在已准备账号的目标设备执行会产生服务端或本地状态变化的 UI 操作，并通过
截图和日志确认提交结果。该脚本不是只读 smoke，运行前必须确保账号数据可重置。
...
不清空运行前日志；仅适用于需要保留跨阶段上下文的排查。

.EXAMPLE
pwsh -File scripts/fast_real_device_write_flows.ps1
#>
param(
...
function Run-Flow {
    ...
    try {
        ...
        Add-Result -CaseId $CaseId -Device $Device -Serial $Serial -Flow $Flow -Status "TOOL_WARN" -Evidence @() -Assertions @{} -Detail "Write-flow exception: $($_.Exception.Message)" -DurationSeconds ([int]((Get-Date) - $start).TotalSeconds)
    }
}
...
function Ensure-BottomTab {
    ...
    for ($attempt = 0; $attempt -lt 4; $attempt++) {
        $state = Capture-State -Adb $Adb -Serial $Serial -Label $DeviceLabel -Suffix "write-nav" -DeviceDir $script:DeviceDir
...
function Open-PrivateChat {
    ...
    $list = Capture-State -Adb $Adb -Serial $Serial -Label $DeviceLabel -Suffix "write-private-list" -DeviceDir $script:DeviceDir
...
function Open-GroupChat {
    param([string]$Adb, [string]$Serial, [string]$DeviceLabel)
    Ensure-BottomTab -Adb $Adb -Serial $Serial -Label $script:T.Messages -XRatio 0.22 -DeviceLabel $DeviceLabel
    $list = Capture-State -Adb $Adb -Serial $Serial -Label $DeviceLabel -Suffix "write-group-list" -DeviceDir $script:DeviceDir
    ...
    if (-not $entry) {
        ...
        if ($groupFilter) {
            ...
            Start-Sleep -Milliseconds $script:DelayMs
            $list = Capture-State -Adb $Adb -Serial $Serial -Label $DeviceLabel -Suffix "write-group-filter" -DeviceDir $script:DeviceDir
            $entry = Find-NodeByDesc -XmlText $list.XmlText -Text "Codex qaGroup" -Contains -ClickableOnly
        }
    }
    ...
    Tap-Node -Adb $Adb -Serial $Serial -Node $entry | Out-Null
    Start-Sleep -Milliseconds $script:DelayMs
    $groupState = Capture-State -Adb $Adb -Serial $Serial -Label $DeviceLabel -Suffix "write-group-opened" -DeviceDir $script:DeviceDir
...
function Send-TextInCurrentChat {
    ...
    Start-Sleep -Milliseconds ([Math]::Max(900, $script:DelayMs * 2))

    $after = Capture-State -Adb $Adb -Serial $Serial -Label $DeviceLabel -Suffix "$Suffix-after-send" -DeviceDir $script:DeviceDir
    $visible = Test-VisibleMessageOutsideInput -XmlText $after.XmlText -Text $Text
    return [pscustomobject]@{
        Visible = $visible
        UsedSendButton = ($null -ne $sendNode)
        Evidence = @($before.PngPath, $before.XmlPath, $typed.PngPath, $typed.XmlPath, $after.PngPath, $after.XmlPath)
    }
}
...
$runTs = New-Timestamp
$privateText = "codexpriv$runTs"
$groupText = "codexgroup$runTs"
$script:Results = New-Object System.Collections.Generic.List[object]

Run-Flow -CaseId "WRITE-B-PRIVATE-TEXT-SEND" -Device "B-UQG" -Serial $DeviceB -Flow "Private chat text send" -Body {
    Open-PrivateChat -Adb $adb -Serial $DeviceB -DeviceLabel "B-UQG"
    $send = Send-TextInCurrentChat -Adb $adb -Serial $DeviceB -DeviceLabel "B-UQG" -Text $privateText -Suffix "write-private"
    $status = if ($send.Visible) { "PASS" } else { "FAIL" }
    New-FlowOutput -Status $status -Evidence $send.Evidence -Assertions @{ sent_text = $privateText; message_visible_after_send = $send.Visible; used_send_button = $send.UsedSendButton } -Detail "Sent timestamped text to Smoke Alice test chat."
}

Run-Flow -CaseId "WRITE-B-GROUP-TEXT-SEND" -Device "B-UQG" -Serial $DeviceB -Flow "Group chat text send" -Body {
    Open-GroupChat -Adb $adb -Serial $DeviceB -DeviceLabel "B-UQG"
    $send = Send-TextInCurrentChat -Adb $adb -Serial $DeviceB -DeviceLabel "B-UQG" -Text $groupText -Suffix "write-group"
    $status = if ($send.Visible) { "PASS" } else { "FAIL" }
    New-FlowOutput -Status $status -Evidence $send.Evidence -Assertions @{ sent_text = $groupText; message_visible_after_send = $send.Visible; used_send_button = $send.UsedSendButton } -Detail "Sent timestamped text to Codex qaGroup test chat."
}

$finalTs = New-Timestamp
$bLog = Join-Path $logDir "B-UQG-write-flows-final-$finalTs.log"
Save-Logcat -Adb $adb -Serial $DeviceB -Path $bLog

$scan = [ordered]@{
    generated_at = Get-IsoNow
    package = $PackageName
    suite = "write-flows-real-device"
    device = "B-UQG"
...
$scanPath = Join-Path $base "write-flows-final-logcat-scan-$finalTs.json"
$scan | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $scanPath -Encoding UTF8
Add-Result -CaseId "WRITE-FINAL-LOGCAT-SCAN" -Device "B-UQG" -Serial $DeviceB -Flow "Write flows final logcat scan" -Status $scanStatus -Evidence @($scanPath, $bLog) -Assertions @{ fatal_pattern_count = $fatalTotal; app_error_pattern_count = $appTotal } -Detail "fatal=$fatalTotal app_error=$appTotal" -DurationSeconds 1

$resultPath = Join-Path $base "write-flows-results-$finalTs.json"
...
$latestSummaryJsonPath = Join-Path $base "latest-write-flows-summary.json"
$latestSummaryMdPath = Join-Path $base "latest-write-flows-summary.md"
...
$summary = [ordered]@{
    ...
    authoritative = $true
    suite = "write-flows-real-device"
    script = "scripts\fast_real_device_write_flows.ps1"
    command = "powershell -NoProfile -ExecutionPolicy Bypass -File scripts\fast_real_device_write_flows.ps1"
    package = $PackageName
    device = $DeviceB
...
$md.Add("# Latest Write Flows Real-Device Suite")
$md.Add("")
...
$md.Add("")
$md.Add("| Case | Device | Result | Flow | Evidence |")
$md.Add("| --- | --- | --- | --- | --- |")
foreach ($r in $latestRows) {
    $md.Add("| $($r.case_id) | $($r.device) | $($r.status) | $($r.flow) | $($r.first_evidence) |")
}
Set-Content -LiteralPath $latestSummaryMdPath -Value ($md -join [Environment]::NewLine) -Encoding UTF8

$reportLines = New-Object System.Collections.Generic.List[string]
$reportLines.Add("")
$reportLines.Add("## Write Flows Real-Device Automation Suite ($((Get-Date).ToString('yyyy-MM-dd HH:mm')) +08:00)")
$reportLines.Add("")
$reportLines.Add("Script: `scripts/fast_real_device_write_flows.ps1`. This suite performs low-risk writes only: timestamped text messages to existing test private/group chats. It does not submit payment, withdraw, recharge, transfer, red packet, delete, forward, or file upload actions.")
$reportLines.Add("")
