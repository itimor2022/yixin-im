...
> IM 客户端在 macOS 端的原生宿主工程，承载 Flutter 引擎与「壹信」应用在 Apple 电脑上的运行，提供窗口、沙盒与系统能力（网络、相机、麦克风、钥匙串等）。
...
- **应用入口与配置**：`AppDelegate.swift`、`MainFlutterWindow.swift`、`Info.plist`、Bundle ID（`com.yixinim.app`）、应用名「壹信」、最低系统版本（macOS 11.0）等。
...
- **应用标识**：`Runner/Configs/AppInfo.xcconfig` 中 PRODUCT_BUNDLE_IDENTIFIER = com.yixinim.app；Info.plist 中 CFBundleName/CFBundleDisplayName「壹信」。
...
| 文件/配置           | 说明 |
|---------------------|------|
| **Runner/Info.plist** | 应用名「壹信」、版本（FLUTTER_BUILD_NAME/NUMBER）、LSMinimumSystemVersion（MACOSX_DEPLOYMENT_TARGET）。 |
...
| **Bundle ID / 应用名** | `Runner/Configs/AppInfo.xcconfig` 中 PRODUCT_BUNDLE_IDENTIFIER；Info.plist 中 CFBundleName/CFBundleDisplayName「壹信」。 |
...
构建完成后可在 `build/macos/Build/Products/Release/` 下找到「壹信.app」，可直接运行或拖入「应用程序」。上架 Mac App Store 或对外分发需在 Xcode 中 Product → Archive，再 Distribute App，并完成公证（Notarization）与签名。
