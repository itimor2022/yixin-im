...
> IM 客户端在 macOS 端的原生宿主工程，承载 Flutter 引擎与「潮商会」应用在 Apple 电脑上的运行，提供窗口、沙盒与系统能力（网络、相机、麦克风、钥匙串等）。
...
| **Bundle ID / 应用名** | `Runner/Configs/AppInfo.xcconfig` 中 PRODUCT_BUNDLE_IDENTIFIER；Info.plist 中 CFBundleName/CFBundleDisplayName「潮商会」。 |
...
构建完成后可在 `build/macos/Build/Products/Release/` 下找到「潮商会.app」，可直接运行或拖入「应用程序」。上架 Mac App Store 或对外分发需在 Xcode 中 Product → Archive，再 Distribute App，并完成公证（Notarization）与签名。
